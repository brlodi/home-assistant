...
def main():
    ...
    parser.add_argument('-a', '--address', dest='ip_addr',
                        help="IP Address of Xbox One", default='')
    parser.add_argument('-i', '--id', dest='live_id',
                        help="Live ID of Xbox One", default='')
    parser.add_argument('-f', dest='forever',
                        help="Send packets until Xbox is on",
                        action='store_true')
    parser.add_argument('-p', '--pingonly', dest='pingonly',
                        help="Send ping to Xbox One without turning on",
                        action='store_true')
    ...
    if not args.pingonly:
        power_payload = b'\x00' + \
            chr(len(live_id)).encode() + live_id.upper() + b'\x00'
        power_header = b'\xdd\x02\x00' + \
            chr(len(power_payload)).encode() + b'\x00\x00'
    ...
    if ping_result:
        ...
        if not args.forever and not args.pingonly:
            while result not in ("y", "n"):
                result = user_input(
                    "Do you wish to keep trying? (y/n): ").lower()
